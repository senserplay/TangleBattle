...
## 2026-04-27 — fix(ui): крестик — плоский полупрозрачный серый круг

### Запрос
"Сделай не блюр, а просто полупрозрачный серый круг радиусом
равным радиусу прицела."

### Фикс
`scripts/characters/player.gd::_draw_crosshair`:

Заменил triangle-fan градиент на **один `draw_circle`**:
```
draw_circle(ch_pos, s * 1.6, Color(0.4, 0.4, 0.4, 0.5))
```
- Нейтральный серый (0.4, 0.4, 0.4) — не подкрашивает цветной
  крестик игрока.
- Альфа 0.5 — заметная «плата», но карта проглядывает.
- Радиус `s * 1.6` совпадает с длиной линий крестика — диск точно
  оборачивает весь рисуемый символ, ни линии, ни арка не выходят
  за пределы.

### Файлы
- `scripts/characters/player.gd::_draw_crosshair` (+5 / −12)
...
## 2026-04-27 — fix(ui): сплошной радиальный диск под крестиком (вместо кольца)

### Запрос
"Блюр должен быть сплошным кругом в радиусе прицела, чтобы на его
фоне было чётко видно прицел."

### Причина
Прошлая итерация рисовала **кольцевую** виньетку (10 концентрических
арок снаружи `s*2.5..s*7.0`). Сам крестик оставался на ярком фоне —
тёмный «бублик» был только вокруг него, а не под. Юзер хочет чтобы
крестик СИДЕЛ на тёмном диске.

### Фикс
`scripts/characters/player.gd::_draw_crosshair`:

Вместо кольца — **сплошной радиальный градиент-диск** через
`draw_polygon` с per-vertex alpha (treangle-fan):
- 1 центральная точка с `Color(0, 0, 0, 0.55)`.
- 33 точки по окружности радиуса `s * 3.0` (≈36 px) с `α = 0`.
- Линейная интерполяция alpha по треугольникам = плавный радиальный
  fade от 55 % черного в центре до 0 на краю.

Радиус 36 px полностью накрывает рисуемый крестик (его линии
заходят до `s * 1.6` ≈ 19 px) с запасом ~17 px, давая фону
заметное затемнение вокруг крестика без жёсткого края.

Никакого шейдера не нужно — Godot сам интерполирует цвета вершин
полигона по умолчанию.

### Файлы
- `scripts/characters/player.gd::_draw_crosshair` (+11 / −12)
...
## 2026-04-27 — tweak(ui): крестик стал ярче + затемняющая виньетка вокруг

### Запрос
"Прицел всё ещё плохо видно на некоторых картах. Сделай его ярче,
а вокруг — небольшое затемнение / блюр карты."

### Фикс
`scripts/characters/player.gd::_draw_crosshair`:

**Затемняющая виньетка** (псевдо-фокус, не настоящий блюр — настоящий
требовал бы шейдера на отдельном CanvasLayer):
- 10 концентрических чёрных арок от радиуса `s*2.5` (≈30 px) до
  `s*7.0` (≈84 px).
- Bell-curve альфа: пик в середине ленты (~t=0.45), плавный спад к
  обоим краям. Крестик не затрагивается, дальний фон тоже остаётся
  чистым — затемнена только полоска вокруг.

**Ярче крестик:**
- `col = player_color.lightened(0.5)` (было 0.3), `α = 1.0` (было 0.85).
- Outline толще: 5.0 (было 4.0), `α = 0.95` (было 0.7).
- Цветной слой толще: 2.5 (было 2.0).
- Добавлен **glow-кольцо** радиуса `s+2.5` с α=0.35 — мягкое сияние.
- **Белая точка-ядро** в центре (1.5 px, α=0.95) для абсолютного
  контраста на любом фоне.

### Файлы
- `scripts/characters/player.gd::_draw_crosshair` (+22 / −10)
