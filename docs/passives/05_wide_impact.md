...
- Взрывы (yarn_bomb, …).
- Ловушки и облака.
- **Heaven's Wrath**: ширина столбов света растёт, расстояние
  между центрами фиксировано → края лучей сближаются и сливаются,
  а край первого луча приближается к игроку.
- **Portal Gate**: радиус свапа + визуал портала и частиц.

Не влияет на скорость и урон снарядов.

**Совокупный множитель ограничен сверху константой
`RADIUS_MULT_CAP = 5.0`** в `player.gd` (максимум ×5 от базового
радиуса). Дальнейшие пики Wide Impact уже не увеличивают радиус.
